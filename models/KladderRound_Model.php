...
class KladderRound_Model {
	...
	{
		$this->mDbConn = $dbConn;
		$this->mTableName = "round_kenoladder";
	...
	public function registerEmptyRound($arrRoundInfo){
		...
		if ($this->mDbConn->query($strSql) === TRUE) {
			$arrRoundInfo['round_fid'] = $this->mDbConn->insert_id;
			return $arrRoundInfo;
		}
	...
	function registerRound($arrRoundInfo, $arrRoundResult)
	{
		...
		if(!array_key_exists("d", $arrRoundResult) || !array_key_exists("r", $arrRoundResult))
			return 0;
		
		
		//날자읽기
		$strDate = $arrRoundResult['d'];
